...
declare_id!("7nS3eWrEUx9C1nv6XJeySfpcKTLaEU6sS4ZT5nDv3HkS");
...
pub mod senddit {
    ...
    pub fn initialize(ctx: Context<Initialize>) -> Result<()> {
        let senddit = &mut ctx.accounts.senddit;
    ...
    pub fn init_post_store(ctx: Context<InitPostStore>) -> Result<()> {
        ...
        let post_store = &mut ctx.accounts.post_store;
        let authority = &mut ctx.accounts.authority;
    ...
    pub fn post_link(ctx: Context<PostLink>, link: String) -> Result<()> {
        ...
        let authority = &mut ctx.accounts.authority;
        let poster_wallet = &mut ctx.accounts.poster_wallet;
    ...
    pub fn init_comment_store(ctx: Context<InitCommentStore>) -> Result<()> {
        ...
        let senddit = &mut ctx.accounts.senddit;
        let treasury = &mut ctx.accounts.treasury;
        let comment_store = &mut ctx.accounts.comment_store;
        let authority = &mut ctx.accounts.authority;
    ...
    pub fn post_comment(ctx: Context<PostComment>, text: String) -> Result<()> {
        ...
        let authority = &mut ctx.accounts.authority;
        let commenter_wallet = &mut ctx.accounts.commenter_wallet;
        ...
        comment_store.comments = comment_store
            .comments
            .checked_add(1)
            .ok_or(ErrorCode::OverflowUnderflow)?;
...
impl Post {
    pub const LEN: usize =
        DISCRIMINATOR + PUBKEY + STRING_PREFIX + MAX_LINK_SIZE + UNSIGNED_64 + UNSIGNED_64 + BUMP;
}
...
impl Comment {
    pub const LEN: usize =
        DISCRIMINATOR + PUBKEY + STRING_PREFIX + MAX_COMMENT_SIZE + UNSIGNED_64 + BUMP;
}
